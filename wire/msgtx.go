...
func (msg *MsgTx) BtcDecode(buf []byte, pver uint32, enc MessageEncoding) error {
	if len(buf) < 4 {
		return io.EOF
	}
	version := littleEndian.Uint32(buf)
	...
	count, read, err := ReadVarIntBytes(buf[offset:], pver)
	if err != nil {
		return err
	}
	...
	for i := uint64(0); i < count; i++ {
		...
		ti := &txIns[i]
		msg.TxIn[i] = ti
		read, err = readTxIn(buf[offset:], pver, msg.Version, ti)
		if err != nil {
			return err
		}
		offset += read
	...
	for i := uint64(0); i < count; i++ {
		...
		to := &txOuts[i]
		msg.TxOut[i] = to
		read, err = ReadTxOut(buf[offset:], pver, msg.Version, to)
		if err != nil {
			return err
		}
		offset += read
	...
	if flag != 0 && enc == WitnessEncoding {
		for _, txin := range msg.TxIn {
			// For each input, the witness is encoded as a stack
			// with one or more items. Therefore, we first read a
			// varint which encodes the number of stack items.
			witCount, read, err := ReadVarIntBytes(buf[offset:], pver)
			if err != nil {
				return err
			}
			offset += read
			...
			for j := uint64(0); j < witCount; j++ {
				txin.Witness[j], read, err = readScript(
					buf[offset:], pver, maxWitnessItemSize, "script witness item",
				)
				if err != nil {
					return err
				}
	...
	msg.LockTime = littleEndian.Uint32(buf[offset:])
...
func readOutPoint(buf []byte, pver uint32, version int32, op *OutPoint) error {
	if len(buf) < chainhash.HashSize+4 {
		return io.EOF
	}
	op.Hash = *(*chainhash.Hash)(buf[:chainhash.HashSize])
	op.Index = littleEndian.Uint32(buf[chainhash.HashSize:])
	return nil
}
...
func readScript(buf []byte, pver uint32, maxAllowed uint32, fieldName string) ([]byte, int, error) {
	offset := 0
	count, read, err := ReadVarIntBytes(buf[offset:], pver)
	if err != nil {
		return nil, read, err
	...
	if count > uint64(maxAllowed) {
		...
		return nil, read, messageError("readScript", str)
	}

	b := buf[offset : offset+int(count)]
	return b, offset + int(count), nil
}

// readTxIn reads the next sequence of bytes from r as a transaction input
// (TxIn).
func readTxIn(buf []byte, pver uint32, version int32, ti *TxIn) (int, error) {
	offset := 0
	err := readOutPoint(buf, pver, version, &ti.PreviousOutPoint)
	if err != nil {
		return offset, err
	}
	offset += chainhash.HashSize + 4

	var read int
	ti.SignatureScript, read, err = readScript(buf[offset:], pver, MaxMessagePayload,
	...
	ti.Sequence = littleEndian.Uint32(buf[offset:])
...
// (TxOut).
func ReadTxOut(buf []byte, pver uint32, version int32, to *TxOut) (int, error) {
	read, err := readElementBytes(buf, &to.Value)
	if err != nil {
		return 0, err
	}
	offset := 8

	to.PkScript, read, err = readScript(buf[offset:], pver, MaxMessagePayload,

...
func TestUtxoCache_EntrySize(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			views: func() []*UtxoViewpoint {
				view1, view2 := NewUtxoViewpoint(), NewUtxoViewpoint()
				for i := uint32(0); i < 10; i++ {
					...
					view1.addTxOut(op, &wire.TxOut{Value: 10000, PkScript: validP2PKHScript}, false, 10)

					// Spend 4.
					if i < 4 {
						view2.addTxOut(op, &wire.TxOut{Value: 10000, PkScript: validP2PKHScript}, false, 10)
						entry := view2.LookupEntry(op)
						entry.Spend()
					}
				}

				return []*UtxoViewpoint{view1, view2}
			}(),
		...
		},
		{
			...
			views: func() []*UtxoViewpoint {
				...
				for i := uint32(0); i < 500; i++ {
					...
					// Also add everything to view 2 and spend them.
					view2.addTxOut(op, &wire.TxOut{Value: 10000, PkScript: validP2PKHScript}, false, 10)
					entry := view2.LookupEntry(op)
					entry.Spend()
				}

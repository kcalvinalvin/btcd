...
func (idx *AddrIndex) FastBuild(chain *blockchain.BlockChain,
	...
	// Requested interrupts are resumable.  Treat every other failure as
	// unrecoverable and direct the user to remove the partial index before
	// trying again.
	defer func() {
	...
	// Indexes created through the legacy constructor do not have a stable
	// staging directory, so leave them to the incremental catchup path.
	if idx.dataDir == "" {
		return nil
	}
	...
	err = idx.db.View(func(dbTx database.Tx) error {
		var err error
		_, tipHeight, err = dbFetchIndexerTip(dbTx, addrIndexKey)
		return err
	})
	if err != nil {
		return err
...
// NewAddrIndex returns a new address index.  It preserves the original
// constructor behavior and uses incremental catchup because no stable data
// directory is available for fast-build staging.
//

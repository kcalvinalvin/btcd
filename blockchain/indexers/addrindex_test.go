...
// TestAddrIndexConstructors ensures the legacy constructor leaves fast builds
// disabled while the data-directory variant enables staging.
func TestAddrIndexConstructors(t *testing.T) {
	t.Parallel()

	legacy := NewAddrIndex(nil, &chaincfg.MainNetParams)
	require.Empty(t, legacy.dataDir, "legacy constructor data directory")
	require.NoError(t, legacy.FastBuild(nil, nil), "legacy FastBuild")
...
func TestAddrIndexFastBuildEndToEnd(t *testing.T) {
	...
	stagingDir, exists := AddrIndexFastBuildStaging(rootDir)
	require.Falsef(t, exists, "completed build left staging at %s", stagingDir)

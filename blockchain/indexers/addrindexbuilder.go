...
// addrBuildManifestMagic identifies the scan checkpoint manifest.
...
// addrSpillShard is one staging file that records are appended to during a
// build.
...
// addrSpiller partitions address index records into staging shards keyed by the
// first byte of the address hash160, then merges them into the index.  Its add
// method is safe for concurrent use across shards.
...
func readAddrSpillShard(f *os.File) ([]addrRecord, error) {
	...
	if len(data)%addrRecordSize != 0 {
		return nil, fmt.Errorf("address index spill shard has a truncated record")
	}

	records := make([]addrRecord, len(data)/addrRecordSize)
	for i := range records {
		off := i * addrRecordSize
		copy(records[i].addrKey[:], data[off:off+addrKeySize])
		records[i].blockID = byteOrder.Uint32(data[off+addrKeySize:])
		records[i].txStart = byteOrder.Uint32(data[off+addrKeySize+4:])
		records[i].txLen = byteOrder.Uint32(data[off+addrKeySize+8:])
	}
	return records, nil
}

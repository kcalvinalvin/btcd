...
func (b *BlockChain) connectBlock(node *blockNode, block *btcutil.Block,
	...
	b.stateLock.Lock()
	if err := b.utxoCache.Commit(view, node); err != nil {
		return err
	}
	b.stateLock.Unlock()
...
func (b *BlockChain) reorganizeChain(detachNodes, attachNodes *list.List) error {
	...
	for i, e := 0, detachNodes.Front(); e != nil; i, e = i+1, e.Next() {
		// Use a new view each time as a view sticking around for longer
		// than a single block breaks the assumptions that utxocache makes
		// for adding/spending entries.
		//
		// When commiting the view to the utxo cache, normally the view is
		// no longer referenced by anything so it gets gc-ed.  Here however,
		// the view will still be referenced since we've made one in the
		// beginning of the reorganizeChain function.  This causes some entries
		// that are not fresh or modified to stay in the view and attempted to be
		// commited twice to the utxo cache.
		//
		// To prevent that problem, just make a new view on every block.
		view = NewUtxoViewpoint()
		...
		err = view.disconnectTransactions(block, detachSpentTxOuts[i], b.utxoCache)
		if err != nil {
			return err
		}

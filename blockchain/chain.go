...
func (b *BlockChain) reorganizeChain(detachNodes, attachNodes *list.List) error {
	...
	for i, e := 0, detachNodes.Front(); e != nil; i, e = i+1, e.Next() {
		...
		// Flush the cache again after the disconnect block because the
		// utxo cache briefly becomes accessible due to the chainLock
		// being unlocked for the notification callback.
		//
		// This is needed as we cannot leave the utxo set in an inconsistent
		// state during block disconnects.
		err = b.db.Update(func(dbTx database.Tx) error {
			return b.utxoCache.flush(dbTx, FlushRequired, b.BestSnapshot())
		})
		if err != nil {

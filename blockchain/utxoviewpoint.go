...
// calcInputsToFetch goes through all the blocks and returns all the outpoints of
...
// outpoints.
func (view *UtxoViewpoint) calcInputsToFetch(block *btcutil.Block) []wire.OutPoint {
...
func (view *UtxoViewpoint) fetchInputUtxos(db database.DB, cache *utxoCache, block *btcutil.Block) error {
	if cache != nil {
		return view.fetchUtxosFromCache(cache, view.calcInputsToFetch(block))
	}
	// Request the input utxos from the cache.
	return view.fetchUtxosMain(db, view.calcInputsToFetch(block))
}

...
func (entry *UtxoEntry) memoryUsage() uint64 {
	...
	// We take the length of the pkscript instead of the cap as all the
	// pkscripts are allocated as a giant blob of byte slice in BtcDecode
	// in msgtx.go.  The pkscript we refer to here is referring to a
	// particular region in that slice.
...
// getEntries returns the UTXO entry for the given outpoint.  It returns nil if
// there is no entry for the outpoint in the view.
//
// This method is part of the utxoView interface.
func (view *UtxoViewpoint) getEntries(outpoints []wire.OutPoint) ([]*UtxoEntry, error) {
	entries := make([]*UtxoEntry, len(outpoints))
	for i := range entries {
		entries[i] = view.LookupEntry(outpoints[i])
	}

	return entries, nil
}

// addEntry adds the given entry to the view.
//
// This method is part of the utxoView interface.
...
func (view *UtxoViewpoint) addInputUtxos(source *utxoCache, block *btcutil.Block) error {
	...
	var outPointsToFetch []wire.OutPoint
	for i, tx := range transactions[1:] {
		for _, txIn := range tx.MsgTx().TxIn {
			...
			// the UTXO view.
			outPointsToFetch = append(outPointsToFetch, txIn.PreviousOutPoint)
		}
	}

	// Fetch the set of entries in batch from the inner utxo view so we can
	// populate this parent view.
	entries, err := source.getEntries(outPointsToFetch)
	if err != nil {
		return err
	}
	...
	for i, entry := range entries {
		view.entries[outPointsToFetch[i]] = entry.Clone()
	}
...
func (view *UtxoViewpoint) connectTransaction(tx *btcutil.Tx, blockHeight int32, stxos *[]SpentTxOut) error {
	...
	if !IsCoinBase(tx) {
		...
		for _, txIn := range tx.MsgTx().TxIn {
			...
			entries, err := view.getEntries([]wire.OutPoint{txIn.PreviousOutPoint})
			if err != nil {
				return err
			}
			entry := entries[0]
...
func (view *UtxoViewpoint) disconnectTransactions(block *btcutil.Block, stxos []SpentTxOut, cache *utxoCache) error {
	...
	for txIdx := len(transactions) - 1; txIdx > -1; txIdx-- {
		...
		for txInIdx := len(tx.MsgTx().TxIn) - 1; txInIdx > -1; txInIdx-- {
			...
			// will never run.
			if stxo.Height == 0 {
				utxo, err := cache.getEntryByHash(txHash)
				if err != nil {
					return err
				}
				if utxo == nil {

...
func ExtractMerkleBranch(merkles []*chainhash.Hash, txHash chainhash.Hash) []*chainhash.Hash {
	...
	// Look for the tx in the leaves. Leaves past the last transaction are
	// nil padding when the count is not a power of two, so skip them.
	idx := -1
	for i := 0; i < numLeaves; i++ {
		if merkles[i] != nil && *merkles[i] == txHash {
			idx = i
			break
		}
	}

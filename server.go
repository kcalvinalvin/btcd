...
// genElectrumCertPair generates a key/cert pair for the electrum server to
// the paths provided. The pair is generated by the electrum package so it
// uses a curve every electrum client TLS stack accepts.
func genElectrumCertPair(certFile, keyFile string) error {
	srvrLog.Infof("Generating TLS certificates for the electrum server...")
	...
	srvrLog.Infof("Done generating TLS certificates for the electrum server")
	return nil
}
...
// generating the pair with the provided genCert function when neither file
// exists. It is used by both the RPC server and the electrum server.
func setupListeners(listenAddrs []string, tlsOn bool, certFile, keyFile string,
	genCert func(certFile, keyFile string) error) ([]net.Listener, error) {
...
func newServer(listenAddrs, agentBlacklist, agentWhitelist []string,
	...
	if cfg.EnableElectrum {
		...
		tlsListener, err := setupListeners(
			...
			cfg.ElectrumKey, genElectrumCertPair)
		if err != nil {
			return nil, err
		}
